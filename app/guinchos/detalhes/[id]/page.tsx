...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const serviceLabel = oficina.country === 'BR' ? 'Guincho & Reboque' : 'Reboque & Auto-Socorro';
  const title = `${oficina.nome} · ${serviceLabel} em ${local || 'Portugal'} · RecarGarage`;
  const description = (oficina.descricao || `Serviço de guincho e pronto-socorro em ${local || 'Portugal'}.`).slice(0, 200);
  ...
  return {
    title,
    description,
    alternates: { canonical: `/guinchos/detalhes/${id}` },
    openGraph: {
      type: 'article',
      title,
      description,
      url: `/guinchos/detalhes/${id}`,
