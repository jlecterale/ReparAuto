/**
 * Pure logic for the side-by-side vehicle comparison (GAP-16 / plan 3.2).
 * Selection state lives in useCompare; this module only derives data.
 */
...
export function buildCompareRows(carros: readonly Carro[]): CompareRow[] {
  ...
  return [
    numericRow('Preço', (c) => c.preco, (v) => formatarPreco(v), 'min'),
    numericRow('Ano', (c) => c.anoFabricacao, (v) => String(v), 'max'),
    numericRow('Quilómetros', (c) => c.km, (v) => `${v.toLocaleString('pt-PT')} km`, 'min'),
