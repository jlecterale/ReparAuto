...
- **AI features (plan 4.1)**: the client NEVER talks to the model — everything goes through the `generateDescription`/`suggestPrice`/`analyzeDamage` callables (`functions/src`), which own auth (verified email), the weekly quota (`aiUsage`, server-derived window key, transaction), input sanitization, image moderation, structured output and the clamp/whitelist repair pass. Client `src/lib/ia/` holds only the UX quota mirror, the idempotency cache and error mapping. `aiUsage`/`aiModerationLog` are server-only collections; `cars.damageAnalysis` is written only by the Cloud Function (rules block owner writes to it). Deviation from the plan prompt: the damage overlay renders in a dedicated panel on the detail page (`DamageAnalysisButton` + `DamageOverlay`) instead of inside `GalleryModal` — the zoom/pan transform there would desync the fractional bounding boxes. The model provider is the **Gemini Developer API** via `@google/genai` (NOT Vertex): the API key is a server-only Firebase secret (`GEMINI_API_KEY`, bound to each callable via `secrets: [GEMINI_API_KEY]`) — never in any client bundle; the Developer API has no EU region pinning (accepted trade-off vs Vertex). App Check is wired but `enforceAppCheck: false` until >95% verified traffic; the last cost defense is the per-day Generative Language API quota cap in the AI Studio / Cloud console.
