...
interface VerifyEmailGateProps {
  /** Leading sentence explaining why a verified email is needed. */
  message?: string;
  /** Renders without the close button / top inset, for use under a native header. */
  inline?: boolean;
}

/**
 * Block shown in place of email-gated flows (listing forms, account
 * verification) when a signed-in user hasn't verified their email. Those
 * writes require a verified email (Firestore rules), so instead of letting
 * the submit fail with a cryptic error we explain the requirement up front
 * and offer resend / re-check actions. Once the email is verified the parent
 * reactively swaps this for the real flow.
 */
export function VerifyEmailGate({
  message = 'Para publicar anúncios precisa de confirmar o seu endereço de email',
  inline = false,
  ...
  return (
    <View className="flex-1 bg-neutral-50" style={inline ? undefined : { paddingTop: insets.top }}>
      {!inline && (
