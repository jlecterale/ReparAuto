 ...
 * Raw provider/internal errors are never surfaced to the user (plan 4.1 §2.5).
 * Mirrors the web `src/lib/ia/aiErrors.ts`.
 */
...
/** True when the error is the weekly-quota rejection (used to zero the UX mirror). */
export function isQuotaError(error: unknown): boolean {
  return normalizeCode(error) === 'resource-exhausted';
}
...
export function mapAiErrorToMessage(error: unknown): string {
  return MESSAGES[normalizeCode(error)] ?? FALLBACK;
}
